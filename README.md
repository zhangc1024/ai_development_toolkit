...
依赖随构建打包，不引用在线字体、外部脚本或转换接口。开发安装依赖需要网络；静态工具的输入只在浏览器内存和本地 Worker 之间传递；提示词优化启用 Ollama 后以及本地 AI 聊天会发送内容到用户配置的本机或内网服务。
...
- 响应式布局；除本地 AI 聊天会保存会话外，其他工具刷新不保存输入或结果。
...
新增整体裁剪、拖动与双指缩放，按原图格式串行导出九张 1080×1080 图片（PNG / JPG / WebP，保留透明区域），支持逐张保存和 ZIP 下载。入口：图片工具 → 一图切九格（`#/moments-grid`）。详见 [使用与实现说明](docs/MOMENTS_GRID.md)。

## 图片加水印

新增文字水印字号、透明度、拖动、旋转和独立复制，预设黑／白／红／黄，支持色号及英文颜色名；可直接复用现有压缩功能，生成后复制图片或下载。入口：图片工具 → 图片加水印（`#/watermark`）。详见 [使用与验证说明](docs/WATERMARK.md)。
...
首页和具体工具页统一使用“首页 + 八类工具”的左侧导航；进入工具后高亮其所属分类，点击分类返回该分类的工具列表。

## 本地 AI 聊天与统一配置

新增 AI 工具 → 本地 AI 聊天（`#/ai-chat`），支持 Ollama 纯文字多轮对话、流式回复、停止生成、复制与会话管理。网站右上角「AI 设置」统一维护 Ollama 地址和默认模型，提示词优化共用，旧配置自动迁移。

连接配置保存在 localStorage，聊天记录保存在 IndexedDB，刷新可恢复；仅当前浏览器和当前网站可用。详见 [使用、保存规则及验证记录](docs/LOCAL_AI_CHAT.md)。