...
import { inspectImage, LEVELS, MIME, outputFilename, TIMEOUT_MS, type CompressionLevel, type ImageFormat, type TargetFormat } from './compression'
...
const source = ref<{ name: string; bytes: ArrayBuffer; url: string; width: number; height: number; format: ImageFormat }>()
const result = ref<{ buffer: ArrayBuffer; url: string; format: ImageFormat; keptOriginal: boolean; notes: string[] }>()
...
async function load(file: File) {
  ...
  try {
    ...
    if (version !== revision) return
    const info = inspectImage(new Uint8Array(bytes))
    const checked = await task({ action: 'inspect', buffer: bytes, level: level.value, target: 'original' })
    if (version !== revision) return
    const url = URL.createObjectURL(new Blob([bytes], { type: MIME[info.format] }))
    source.value = { name: file.name || '粘贴图片.' + info.format, bytes, url, width: checked.width, height: checked.height, format: info.format }
...
async function compress() {
  ...
  try {
    const data = await task({ action: 'compress', buffer: source.value.bytes, level: level.value, target: format.value })
    if (version !== revision) return
    result.value = { ...data, url: URL.createObjectURL(new Blob([data.buffer], { type: MIME[data.format as ImageFormat] })) }
  ...
  <div class="tool-page compression-tool">
    ...
    <section class="time-form">
      <div class="drop-zone" @dragover.prevent @drop.prevent="drop">
        <label>选择图片或 ZIP<input ref="input" type="file" accept="image/png,image/jpeg,image/webp,.zip,application/zip" @change="select" /></label>
        <p>支持拖放或粘贴 · 静态 PNG / JPG / WebP ≤ 10 MiB · ZIP ≤ 50 MiB</p>
      </div>
      <div class="encoding-options-row">
        ...
        <label class="indent-label">目标格式
          <select v-model="format">
            <option value="original">原格式</option><option value="webp">WebP</option>
    ...
    <section class="encoding-rules">
      <h2>使用说明</h2>
      <ul>
        <li>不缩放图片；有方向信息的照片按浏览器显示方向处理。仅支持静态 PNG/JPG/WebP，拒绝 GIF、APNG、动态 WebP 和多图片 JPEG。</li>
        <li>原格式结果不小于原文件时保留原文件；主动转换格式时仍可下载转换结果。</li>
        <li>PNG 的有损档会尝试减少颜色数量；未达到质量要求时回退无损结果。较强档位会比较多个候选，选择更小文件。</li>
        <li>PNG 原格式保真档直接优化原始数据；JPG/WebP 原格式保真档保留原文件。其他重新编码可能转为 8 位 sRGB，并移除 EXIF 等元数据，广色域或 16 位素材请优先保留原格式保真档。</li>
        <li>单图模式每次一张，单边最多 16384 像素，处理最多 60 秒。修改选项会取消任务并清除旧结果；离开页面释放图片。</li>
